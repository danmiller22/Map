...
async function refresh(){
  ...
  for(const p of pairs){
    const tr=p.trailer;
    if(tr && tr.position){
      trailers.push({type:'Feature',geometry:{type:'Point',coordinates:[tr.position.lon,tr.position.lat]},properties:{id:tr.id,label:'TR '+tr.id}});
    }
    if(p.truck && p.truck.position){
      trucks.push({type:'Feature',geometry:{type:'Point',coordinates:[p.truck.position.lon,p.truck.position.lat]},properties:{id:p.truck.id,label:'TK '+p.truck.id}});
      if(tr && tr.position){
        lines.push({type:'Feature',geometry:{type:'LineString',coordinates:[[p.truck.position.lon,p.truck.position.lat],[tr.position.lon,tr.position.lat]]},properties:{id:(tr.id+'-'+p.truck.id),distance:p.distanceMiles}});
      }
    }
  }
  ...
  for(const p of pairs){
    ...
    li.addEventListener('click',()=>{ if(p.truck && p.truck.position && p.trailer && p.trailer.position){
      const b=new maplibregl.LngLatBounds(); b.extend([p.truck.position.lon,p.truck.position.lat]); b.extend([p.trailer.position.lon,p.trailer.position.lat]);
      map.fitBounds(b,{padding:60,maxZoom:12});
    } else if(p.trailer && p.trailer.position){
...
function setOrUpdate(id,data,img){
  if(map.getSource(id)){ map.getSource(id).setData(data); }
    ...
    map.addLayer({id,type:'symbol',source:id,layout:{'icon-image':img,'icon-size':1,'icon-allow-overlap':true,'text-field':['get','label'],'text-offset':[0,1.2],'text-size':10,'text-anchor':'top'}});
  }
}
